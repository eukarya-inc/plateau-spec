...
| 型の定義 | 地物の外観。 地物の幾何オブジェクトとなる面に色又はテクスチャを指定する。![Image](images/toc4_22_03_01_img001.png)Apperanceを使用してbldg:Buildingの外観を設定した例　　　　（左：色の指定　右：テクスチャの指定） | ← |
| --- | --- | --- |
| 上位の型 | gml:_Feature | ← |
| ステレオタイプ | << FeatureType >> | ← |
| 継承する属性 | ← | ← |
...
| 型の定義 | 地物インスタンスに貼り付ける画像と貼り付け方をパラメータにより指定するための型。![Image](images/toc4_22_03_01_img002.png)bldg:Buildingの外観としてテクスチャを指定した例 | ← |
| --- | --- | --- |
| 上位の型 | app:_Texture | ← |
| ステレオタイプ | << FeatureType >> | ← |
| 継承する属性 | ← | ← |
...
| 型の定義 | 地物インスタンスの表面色を指定するための型。![Image](images/toc4_22_03_01_img003.png)bldg:Buildingの外観として色を指定した例 | ← |
| --- | --- | --- |
